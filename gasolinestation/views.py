...
class SalesView(LoginRequiredMixin, TemplateView):
    ...
    def get(self, request):
        ...
        weekly_sales = TransactionSales.objects.filter(created_at__week=52).aggregate(Sum('sales'))
        yearly_sales = TransactionSales.objects.filter(created_at__year=yearly).aggregate(Sum('sales'))
        ...
        weekly_liter = TransactionSales.objects.filter(created_at__week=52).aggregate(Sum('dispensed_liter'))
        yearly_liter = TransactionSales.objects.filter(created_at__year=yearly).aggregate(Sum('dispensed_liter'))
        ...
        weekly_price = TransactionSales.objects.filter(created_at__week=52).aggregate(Sum('price'))
        yearly_price = TransactionSales.objects.filter(created_at__year=yearly).aggregate(Sum('price'))
